...
public function buscador_participantes_cupones($data){
          ...
          /*$this->db->select( 'GROUP_CONCAT(DISTINCT(   CONCAT("<span style=color:red>", c.valor, "</span> [",( CASE WHEN c.fecha_participacion = 0 THEN "" ELSE DATE_FORMAT(FROM_UNIXTIME(c.fecha_participacion),"%d-%m-%Y %H:%i:%s") END ),"]  "  )    ) ORDER BY c.id SEPARATOR "|") as cupones',false);*/

           //  (3600*6hrs) es 21600
           $this->db->select( 'GROUP_CONCAT(DISTINCT(   CONCAT("<span style=color:red>", c.valor, "</span> [",( CASE WHEN (c.fecha_participacion-21600) = 0 THEN "" ELSE DATE_FORMAT(FROM_UNIXTIME( (c.fecha_participacion-21600)  ),"%d-%m-%Y %H:%i:%s") END ),"]  "  )    ) ORDER BY c.id SEPARATOR "|") as cupones',false);

          //$this->db->select("( CASE WHEN c.fecha_participacion = 0 THEN "" ELSE DATE_FORMAT(FROM_UNIXTIME(c.fecha_participacion),"%d-%m-%Y %H:%i:%s") END ) AS fecha_participacion", FALSE); 
              ...
              if ( $result->num_rows() > 0 ) {
                  ...
                  foreach ($result->result() as $row) {
                               $dato[]= array(
                                      ...
                                      1=>$row->email,
                                      2=>$row->cupones, //.$row->fecha_participacion,
                                    );
                      }
...
public function buscador_catalogos($data){
      ...
      public function validar_catalogo( $data ){
            
            if  ($data['id_estatus']==1) {
              $this->db->from($this->catalogo_importacion.' p');  
            } else { //caso de 2
                $this->db->from($this->catalogo_exportacion.' p');  
            }


            $where = '( 
                (p.id_puerto ='.$data['id_puerto'].') and  
                (p.id_destino ='.$data['id_destino'].') and  
                (p.id_puertoescala ='.$data['id_puertoescala'].') and  
                (p.id_puertoescala2 ='.$data['id_puertoescala2'].') 
            )';
            $this->db->where($where);  
            $result = $this->db->get();

              if ($result->num_rows() > 0){
                return false; //return ($result->result());
              }
              else {
                return true;
              }
      ...
      public function nuevo_catalogo( $data ){
            $timestamp = time();
...
public function buscador_puertos($data){
          ...
          $this->db->select("SQL_CALC_FOUND_ROWS *", FALSE); //
           ...
           $this->db->select('id, puerto, city, city_ascii, lat, lng, pop, country, iso2, iso3, province');

          
            
            $this->db->from($this->catalogo_puerto.' e');  
       ...
       //editar 
        public function get_puerto( $data ){
    ...
    public function edicion_puerto( $data ){

            $timestamp = time();

            $id_session = $this->session->userdata('id');
                 
            $this->db->set( 'puerto', $data['puerto'] );
            ...
            $this->db->where('id', $data['id'] );
             ...
             $this->db->update($this->catalogo_puerto );
    ...
    public function nuevo_puerto( $data ){
            ...
            $this->db->set( 'country', $data['country'] );
             ...
             $this->db->insert($this->catalogo_puerto );
