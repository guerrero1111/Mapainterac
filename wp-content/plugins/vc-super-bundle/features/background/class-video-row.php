	...
	class GambitVCVideoRow {
		...
		public function create_shortcode( $atts, $content = null ) {
			...
			if ( 'youtube' == $video_meta['type'] ) {
				...
				$video_div .= "<div style='visibility: hidden' id='video-" . self::$video_id . "' data-youtube-video-id='" . esc_attr( $video_meta['id'] ) . "' data-force-hd='" . ( 'forcehd' == $atts['force_hd'] ? 'true' : 'false' ) . "' data-mute='" . ( 'mute' == $atts['mute'] ? 'true' : 'false' ) . "' data-video-aspect-ratio='" . esc_attr( $atts['aspect_ratio'] ) . "' >";
				...
				$video_div = '<div class="click-overrider"></div>';
				$video_div .= '<div id="video-' . self::$video_id . '" data-vimeo-video-id="' . esc_attr( $video_meta['id'] ) . '" data-mute="' . ( 'mute' == $atts['mute'] ? 'true' : 'false' ) . '" data-video-aspect-ratio="' . esc_attr( $atts['aspect_ratio'] ) . '">';
				$video_div .= '<iframe id="video-iframe-' . self::$video_id . '" src="https://player.vimeo.com/video/' . $video_meta['id'] . '?api=1&player_id=video-iframe-' . self::$video_id . '&html5=1&autopause=0&autoplay=1&badge=0&byline=0&loop=1&title=0&background=1" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
				$video_div .= '</div>';
			}
